...
class TrainOptions(BaseOptions):
	def initialize(self):
		...
		#self.parser.add_argument("--num_classes", default=12, type=int)
		self.parser.add_argument("--num_classes", default=4, type=int)
		#self.parser.add_argument("--name", type=str, default='stack_aug')
		self.parser.add_argument("--name", type=str, default='uavdtaugfine')

		#self.parser.add_argument("--dataroot", type=str, default='dataset/visDrone/augtrain')
		self.parser.add_argument("--dataroot", type=str, default='dataset/UAVDT/augtrain')
		self.parser.add_argument("--dataset_mode", default="Fine", help="coarse, fine, coarseval, fineval, crop")
		...
		self.parser.add_argument("--batch_size", type=int, default=12, help="size of each image batch")

		# load model
		#self.parser.add_argument("--load_model", default='checkpoints/uavdtfine/hourglass_0.pth')
		#self.parser.add_argument("--load_model", default='checkpoints/stack_aug/hourglass_latest.pth')
		...
		self.parser.add_argument("--lr_step", type=str, default='30,100')
		...
		self.parser.add_argument('--stack_num_classes', type=int, default=3, help='stack class for IID')
		self.parser.add_argument('--shm_weight', type=float, default=1, help='loss weight for keypoint heatmap')

		self.parser.add_argument('--augment', action='store_true')
		self.parser.add_argument('--imgpool_path', type=str, default='dataset/visDrone/imgpools')
		self.parser.add_argument('--sample_k', type=int, default=2)
	...
	def parse(self):
		...
		if not self.opt.stack:
			self.opt.heads = {'hm': self.opt.num_classes, 'wh': 2, 'reg': 2}
		else:
			self.opt.heads = {'hm': self.opt.num_classes, 'shm': self.opt.stack_num_classes, 'wh': 2, 'reg': 2}

		if 'UAVDT' not in self.opt.dataroot:
			self.opt.classes = ['ignored regions','prdestrain','person','bicycle','car','van',
				'truck','tricycle','awning tricycle','bus','motor','others']
		else:
			self.opt.classes = ['ignore','car', 'truck', 'bus']
		self.opt.stack_classes = ['human', 'nonvehicle', 'vehicle']
		#self.opt.classes = ['ignore','car', 'truck', 'bus']

		# if self.opt.augment:
		# 	self.opt.bicycle = os.listdir(self.opt.imgpool_path+'')
