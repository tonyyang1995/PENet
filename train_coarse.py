...
from opt.CoarseOptions import TrainOptions
from opt.CoarseTestOptions import TestOptions
	...
	for epoch in range(opt.start_epochs, opt.start_epochs + opt.epochs):
		...
		for i, inputs in enumerate(dataloader):
			...
			if i % opt.display_freq == 0:
				...
				#vis.plot_prediction(imgs, hms, whs, regs)
				...
				#vis.plot_rawdata_target(inputs['img'], inputs['targets'])
				# plot current loss and outputs
				avg_loss = {'loss': 0, 'hm_loss': 0, 'wh_loss': 0, 'off_loss': 0}
				#self.model.save('latest', epoch)
			count += 1
			#break
